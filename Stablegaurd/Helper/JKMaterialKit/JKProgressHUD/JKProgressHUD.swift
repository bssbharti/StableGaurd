...
public class JKProgressHUD:UIView{
    ...
    fileprivate  func updatehudContraints(item:UIView, toItem:UIView){
        ...
        let top : NSLayoutConstraint = NSLayoutConstraint(item: item, attribute: .top, relatedBy: .equal, toItem: toItem, attribute: .top, multiplier: 1, constant: 0)
...
extension UIImage {
    ...
    public class func gif(name: String,withExtension extens : String = "gif") -> UIImage? {
        // Check for existance of gif
        var fileExtension : String
        if extens.caseInsensitiveCompare("gif") == .orderedSame {
             fileExtension = extens
        }
        else{
            fileExtension = "Gif"
        }
