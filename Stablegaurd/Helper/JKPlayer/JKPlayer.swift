...
public typealias JKPlayerFillMode = AVLayerVideoGravity

/// Asset playback states.
public enum JKPlaybackState: Int, CustomStringConvertible {
    case stopped = 0
    case playing
    case paused
    case failed
...
public let JKPlayerErrorDomain = "PlayerErrorDomain"

/// Error types.
public enum JKPlayerError: Error, CustomStringConvertible {
    case failed
...
public protocol JKPlayerDelegate: NSObjectProtocol {
    func playerReady(_ player: JKPlayer)
    func playerPlaybackStateDidChange(_ player: JKPlayer)
    func playerBufferingStateDidChange(_ player: JKPlayer)
    ...
    func player(_ player: JKPlayer, didFailWithError error: Error?)
}


/// Player playback protocol
public protocol JKPlayerPlaybackDelegate: NSObjectProtocol {
    func playerCurrentTimeDidChange(_ player: JKPlayer)
    func playerPlaybackWillStartFromBeginning(_ player: JKPlayer)
    func playerPlaybackDidEnd(_ player: JKPlayer)
    func playerPlaybackWillLoop(_ player: JKPlayer)
}

// MARK: - Player
/// ▶️ Player, simple way to play and stream media
open class JKPlayer: UIViewController {
    
    /// Player delegate.
    open weak var playerDelegate: JKPlayerDelegate?
    
    /// Playback delegate.
    open weak var playbackDelegate: JKPlayerPlaybackDelegate?
    ...
    open var fillMode: JKPlayerFillMode {
        get {
    ...
    open var playbackState: JKPlaybackState = .stopped {
        didSet {
                ...
                for key in loadableKeys {
                    ...
                    if status == .failed {
                        ...
                        self.executeClosureOnMainQueueIfNecessary {
                            self.playerDelegate?.player(self, didFailWithError: JKPlayerError.failed)
                        }
                        return
                    }
                }
...
extension JKPlayer {
    ...
    internal func addPlayerItemObservers() {
        ...
        self._playerItemObservers.append(playerItem.observe(\.playbackBufferEmpty, options: [.new, .old]) { (object, change) in
            ...
            case .failed:
                self.playbackState = JKPlaybackState.failed
            default:
                break
            }
        })
...
public class PlayerView: UIView {
    ...
    public var playerFillMode: JKPlayerFillMode {
        get {
